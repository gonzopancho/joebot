...
public class PIDController implements IUtility{
    ...
    private double m_prevError2 = 0.0;
    ...
    private void calculate() {
        ...
        if (enabled) {
            ...
            synchronized (this) {
                ...
                double predictedError = m_error + (m_error - m_prevError); // predicted one time step into the future
                double predictedVelocity = 2 * m_error - 3 * m_prevError - m_prevError2;
                if (m_continuous) {
                    if (Math.abs(m_error)
                ...
                m_result = (m_P * predictedError + m_I * (m_totalError + predictedError) + m_D * predictedVelocity);
                m_prevError2 = m_prevError;
                m_prevError = m_error;
